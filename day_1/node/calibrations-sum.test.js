...
it("should convert number letter to digit", () => {
  ...
  assert.strictEqual(convertLettersToNumbers("144"), "144");
});
...
it("should calculate the calibrations sum", () => {
  const tests = [
    {
      values: ["dskdskds7dd7", "hehe110", "dkdk15"],
      result: 102,
    },
    {
  ...
  for (const calibrationTest of tests) {
    assert.strictEqual(
      calculateCalibrationSum(calibrationTest.values),
      calibrationTest.result,
    );
  }
});
